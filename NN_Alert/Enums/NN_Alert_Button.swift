...
public class NN_Button:RaisedButton{
    ...
    func applyStyle(btnStyle:NN_Btn_Style?  = .dismiss){
        ...
        case .normal :
            ...
            break;
        case .attention :
            self.backgroundColor = NN_Alert_Manager.shared.style.colors.secondColor
            self.setTitleColor(#colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), for: .normal)
            break;
        case .destructive:
            self.backgroundColor = NN_Alert_Manager.shared.style.colors.fourColor
            self.setTitleColor(#colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), for: .normal)
            break;
        case .emphasize:
    ...
    public override func didMoveToSuperview() {
        if type(of: self.superview) == NN_Modal.self{
            self.host_alert = self.superview as? NN_Modal
        }
    }
